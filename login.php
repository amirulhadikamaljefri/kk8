...
<body>
	...
	if ($username && $password) {
		$conn = mysqli_connect("localhost", "root", "","farmasi")// tiada nama db
			or die("Not Connect to the database"); //syntax error
		mysqli_select_db($conn, "farmasi") or die("incorrect database"); //syntax error & tiada tag $conn

		$query = mysqli_query($conn, "SELECT * FROM user WHERE username= '$username'"); //tiada tag $conn
